...
test_btn=tk.Button(master=r_frm,
                   command=function1,
                   text='red checkers'
                   )
test_btn.configure(bg=rb_color, fg='white', activebackground=ra_color, activeforeground='white')
test_btn.pack(side='right',
              ipadx=10
              )

test_btn2=tk.Button(master=r_frm,
                    command=function2,
                    text='make red'
                    )
test_btn2.pack(side='right', ipadx=10)
test_btn2.configure(bg=rb_color, fg='white', activebackground=ra_color, activeforeground='white')

shuffle2_btn=tk.Button(master=r_frm,
                       command=shuffle2,
                       text='shuffle\nswapping'
                       )
shuffle2_btn.pack(side='right')
shuffle2_btn.configure(bg=rb_color, fg='white', activebackground=ra_color, activeforeground='white')

shfl_btn=tk.Button(master=r_frm,
                   command=shuffle,
                   text='shuffle\npasting'
                   )
shfl_btn.pack(side='right')
shfl_btn.configure(bg=rb_color, fg='white', activebackground=ra_color, activeforeground='white')
...
crope_btn=tk.Button(master=l_frm,
                    command=crop_even,
                    text='crop evenly to grid size'
                    )
crope_btn.pack(side='left')
crope_btn.configure(bg=lb_color, activebackground=la_color)

undo_btn=tk.Button(master=l_frm,
                   command=undo,
                   text='undo\n[Ctr+z]'
                   )
...
redo_btn=tk.Button(master=l_frm,
                   command=redo,
                   text='redo\n[Ctr+Shift+z]'
                   )
